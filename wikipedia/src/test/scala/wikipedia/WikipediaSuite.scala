...
class WikipediaSuite {
  ...
  @Test def `'occurrencesOfLang' should work for (specific) RDD with some elements`(): Unit = {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    val rdd = sc.parallelize(Seq(WikipediaArticle("title", "Java Jakarta"), WikipediaArticle("title", "Scala Not Like Java")))
    assert(occurrencesOfLang("Java", rdd) == 2, "occurrencesOfLang given (specific) RDD with should equal to 2")
    assert(occurrencesOfLang("Scala", rdd) == 1, "occurrencesOfLang given (specific) RDD with should equal to 1")
  }

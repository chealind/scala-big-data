...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    ...
    val questionsPart = questions.partitionBy(partitioner).persist()
    val answersPart = answers.partitionBy(partitioner).persist()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langIndexFrequency = vs.map(p => p._1 / langSpread).groupBy(identity).mapValues(_.size)
      val topLangIndex = langIndexFrequency.maxBy(_._2)._1

      // most common language in the cluster
      val langLabel: String = langs(topLangIndex)

      // percent of the questions in the most common language
      val langPercent: Double = langIndexFrequency(topLangIndex) * 100.0d / vs.size

      val clusterSize: Int = vs.size

      val medianScore: Int = getMedianScore(vs.map(_._2).toList.sorted)
  ...
  def getMedianScore(scoreList: List[Int]): Int = {
    val middle = scoreList.size / 2
    if (scoreList.size % 2 != 0) {
      scoreList(middle)
    } else {
      (scoreList(middle - 1) + scoreList(middle)) / 2
    }
  }
